...
- Maintainer: decides what goes in, reviews and merges pull requests, cuts releases and handles security reports. Also administers the repository, the container registries and the Helm chart repository. Currently one person.
...
Two things are frozen unless there is an explicit decision to break them: the exported metric names and labels, and the command line flags.
Both have an installed base, and a rename silently breaks dashboards, alerts and scripts of people who never see the change.
