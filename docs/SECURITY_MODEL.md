...
- The container image runs as root by default for the time being, because changing the execution identity of a widely deployed image is a migration of its own. [#463](https://github.com/utkuozdemir/nvidia_gpu_exporter/issues/463) tracks making non-root the default, and the chart README documents a non-root security context.
- The release artifacts are signed: the GPG-signed checksums file covers every binary, archive and package, and the container images and the Helm chart are signed with cosign. The auto-generated GitHub source archives and the git tags are not covered. [#493](https://github.com/utkuozdemir/nvidia_gpu_exporter/issues/493) tracks signing the tags. See [Verifying what you downloaded](INSTALL.md#verifying-what-you-downloaded).
